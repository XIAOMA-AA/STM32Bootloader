...
static void Int_flash_erase(void)
{
    ...
    if (is_erase)
    {
        ...
        flash_erase_init_struct.Page = page_addr;           // 页地址
...
static void Int_flash_write_with_last(void)
{
    ...
    for (uint16_t i = 0; i < uart_rec_len; i += 2)
    {
        ...
        HAL_FLASH_Program(FLASH_TYPEPROGRAM_DOUBLEWORD, flash_addr, buff);
    }
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t size)
{
    if (huart->Instance == USART2)
    {
        ...
        // 启动下一次接收
        HAL_UARTEx_ReceiveToIdle_IT(&huart2, (uint8_t *)Bootloader_Receive_Buffer, Bootloader_Receive_Buffer_Size);
    }
}
