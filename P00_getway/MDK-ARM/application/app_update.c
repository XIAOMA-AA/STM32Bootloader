...
{
    Int_fdcan_recv(msg, &msg_count);
    for (uint8_t i = 0; i < msg_count; i++)
    {
        // 只接受ID为0的数据
        if (msg[i].rx_header.Identifier == 0)
        {
            // 判断是否为更新消息 "sss"
...
void App_Update_send_app(void)
{
    ...
    if (update_data_len < uart_rec_full_len)
    {
        ...
        Int_fdcan_send(APP_UPDATE_CMD_ID, update_buff, send_len);
    ...
    else
    {
        ...
        Int_fdcan_send(APP_UPDATE_CMD_ID, (uint8_t *)&crc_value, 4);
    }
}
